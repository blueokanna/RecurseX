...
默认只合成 IPv4，`AAAA` 返回 NODATA。配了 `fake-ip-range6` 之后，`AAAA` 会从该段
合成地址（默认值可参考 `DEFAULT_FAKE_IP_RANGE6`，即 `fdfe:dcba:9876::/48`）。
...
* 客户端拿到合成 `AAAA` 就会走 IPv6 连过去。有 v6 池托底时这是对的；
  没有池托底时，那是一个**无法反向的地址**，只能算作静默失效。
* 反过来，不配 v6 段时 NODATA 会把双栈客户端推回已拿到的 v4 地址。

两个地址族**各自独立**：一个名字可以同时持有 v4 与 v6 映射（互不影响），
各自的 `maxEntries`、各自的地址游标、各自的回收计数；一个族的压力不会
淘汰另一族的映射。反向解析两边都支持：`in-addr.arpa` 与 `ip6.arpa`（32 个
半字节标签，低位在后，非规范的标签一律拒绝而不是当成地址）。
